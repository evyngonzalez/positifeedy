...
extension KeychainItem {
    ...
    //Get and Set Current User First Name. Set nil to delete.
       static var store_start_date: String? {
           get {
               return try? KeychainItem(service: bundleIdentifier, account: "store_start_date").readItem()
           }
           set {
               guard let value = newValue else {
                   try? KeychainItem(service: bundleIdentifier, account: "store_start_date").deleteItem()
                   return
               }
               do {
                   try KeychainItem(service: bundleIdentifier, account: "store_start_date").saveItem(value)
               } catch {
                   print("Unable to save userFirstName to keychain.")
               }
           }
