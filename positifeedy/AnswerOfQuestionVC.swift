...
class AnswerOfQuestionVC: UIViewController,UITextViewDelegate,AVAudioRecorderDelegate, AVAudioPlayerDelegate
{
    ...
    func design() -> Void {
        ...
        self.txtcomment.layer.cornerRadius = 3
                                                            ...
                                                            if let strURL = (questionItem.link)
                                                               {
                                                                   ...
                                                                   let url = URL(string: strURL)
                                                                    self.imgItem.sd_setImage(with: url, placeholderImage: UIImage(named: "place"))
    ...
    @IBAction func onclickforStartFromFirst(_ sender: Any) {
        ...
        else
        {
            ...
            self.timer = nil
            self.btnStartRecFirst.setImage(UIImage.init(named: "start_record"), for: .normal)
        }
    }
          ...
          /*  if audioRecorder != nil
            {
                ...
                let final = String.init(format: "%@.m4a", self.randomString(length: 5))
                let storageRef = Storage.storage().reference().child(final)
                storageRef.putFile(from:getFileUrl(), metadata: nil) { (metadata, error) in
...
//                        prepare_play()
//                        audioPlayer.setVolume(4, fadeDuration: 1)
//                        audioPlayer.play()
//                        isPlaying = true
//                    }
//                    else
...
//                    }
        ...
        else
        {
            if self.txtcomment.text == "Write Here.."
                   {
                       self.view.makeToast("Please enter your answer here!")
                   }
                   else
                   {
        ...
        if self.isPlayFlag == 0
        {
            ...
            else
            {
                ...
                if self.strAudiFileName != nil
                {
                            ...
                            {
                                self.view.makeToast("Audio file is missing.")
                    ...
                    if d.count > 0
                    {
                        ...
                        if (d["uid"] as! String)  ==  Auth.auth().currentUser?.uid
                        {
                           ...
                           if let strURL = (d["pic"] as? String)
                           {
                               ...
                               self.imgProfile.sd_setImage(with: url, placeholderImage: UIImage(named: "place"))
