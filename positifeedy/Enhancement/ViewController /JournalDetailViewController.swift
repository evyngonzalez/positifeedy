...
class JournalDetailViewController: UIViewController,UITextViewDelegate,AVAudioRecorderDelegate, AVAudioPlayerDelegate
{
    ...
    override func viewDidLoad()
    {
        ...
        if(IsEditingOn){
            ...
            firebaseaudioURL = DataJournal.value(forKey: "audio_url")as? String ?? ""
            if(firebaseaudioURL == ""){
            ...
            txtcomment.text = message
            
            let question = DataJournal.value(forKey: "question") as? String ?? ""
            lblQuestion.text = question
            
            let imgUrl = DataJournal.value(forKey: "link") as? String ?? ""
            if(imgUrl != ""){
                self.imgItem.sd_setImage(with: URL(string: imgUrl)!, placeholderImage: UIImage(named: "ReflectionBG"))
            }
            
            
            let answerdate = DataJournal.value(forKey: "current_date") as? String ?? ""
            if(answerdate != ""){
                let dateFormatterr = DateFormatter()
                dateFormatterr.dateFormat = "yyyy-MM-dd"
                let date = dateFormatterr.date(from: answerdate) ?? Date()
